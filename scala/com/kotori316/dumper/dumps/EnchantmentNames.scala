...
object EnchantmentNames extends Dumps {
    ...
    override def content(): Seq[String] = {
        val datas = ForgeRegistries.ENCHANTMENTS.getEntries.asScala.toSeq.map(new EData(_)).sorted
        val mn: Int = datas.map(_.e.getName.length).max
        val mi: Int = datas.map(_.name.toString.length).max
        //       index   id    name  Resource maxlevel rarity treasure
        val f = s"%3d : %3d : %-${mn}s : %-${mi}s : %d : %s : %b"
        "number : ID : name : mod : MaxLevel : Rarity : Treasure" +: datas.zipWithIndex
